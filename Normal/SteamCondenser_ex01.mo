...
model SteamCondenser_ex01 "Complete drum boiler model, including evaporator and supplementary components"
  ...
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow heatSink annotation(
  ...
  Modelica.Fluid.Sensors.Temperature T_condenserInlet(redeclare package Medium = Modelica.Media.Water.StandardWater) annotation(
    Placement(visible = true, transformation(origin = {-28, 20}, extent = {{10, 10}, {-10, -10}}, rotation = 180)));
  Modelica.Fluid.Sensors.Pressure p_condenserInlet(redeclare package Medium = Modelica.Media.Water.StandardWater) annotation(
  ...
  Modelica.Fluid.Sources.MassFlowSource_T LiquidScavengePump(redeclare package Medium = Modelica.Media.Water.StandardWater, T = 15 + 273.15, m_flow = 1, nPorts = 1, use_m_flow_in = true) annotation(
    Placement(visible = true, transformation(origin = {70, 0}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  Modelica.Fluid.Sources.Boundary_pT VaporSupplyBoundary(redeclare package Medium = Modelica.Media.Water.StandardWater, T = 120 + 273.15, nPorts = 1, p = 1.5 * 101.325 * 1000) annotation(
  ...
  Modelica.Fluid.Sensors.Temperature T_condenserOutlet(redeclare package Medium = Modelica.Media.Water.StandardWater) annotation(
    Placement(visible = true, transformation(origin = {40, 20}, extent = {{10, 10}, {-10, -10}}, rotation = 180)));
  Modelica.Fluid.Sensors.MassFlowRate massFlowLiquid(redeclare package Medium = Modelica.Media.Water.StandardWater) annotation(
...
equation
  connect(heatSink.port, condenser.heatPort) annotation(
    Line(points = {{2, -19}, {2, -10}}, color = {191, 0, 0}));
  connect(ramp_Q_flow_in.y, heatSink.Q_flow) annotation(
  ...
  connect(VaporValve.port_b, VaporSupplyBoundary.ports[1]) annotation(
  ...
  connect(massFlowVapor.port_a, VaporValve.port_a) annotation(
    Line(points = {{-68, 0}, {-100, 0}}, color = {0, 127, 255}));
  connect(condenser.port_b, massFlowVapor.port_b) annotation(
    Line(points = {{-8, 0}, {-48, 0}}, color = {0, 127, 255}));
  connect(p_condenserInlet.port, massFlowVapor.port_b) annotation(
    Line(points = {{-48, 20}, {-48, 0}}, color = {0, 127, 255}));
  connect(LiquidScavengePump.ports[1], massFlowLiquid.port_b) annotation(
    Line(points = {{60, 0}, {40, 0}}, color = {0, 127, 255}));
  connect(T_condenserOutlet.port, massFlowLiquid.port_b) annotation(
    Line(points = {{40, 10}, {40, 0}}, color = {0, 127, 255}));
  connect(massFlowLiquid.port_a, condenser.port_a) annotation(
    Line(points = {{20, 0}, {12, 0}}, color = {0, 127, 255}));
  connect(T_condenserInlet.port, condenser.port_b) annotation(
    Line(points = {{-28, 10}, {-28, 0}, {-8, 0}}, color = {0, 127, 255}));
  connect(limiter.u, ctrl_pi.y) annotation(
    Line(points = {{60, 70}, {43, 70}}, color = {0, 0, 127}));
  connect(limiter.y, LiquidScavengePump.m_flow_in) annotation(
