...
model TurbojetEngine_byPump_2spool_temp01
  ...
  Modelica.Fluid.Valves.ValveCompressible valveCompressible(redeclare package Medium = Modelica.Media.Air.DryAirNasa, dp_nominal(displayUnit = "Pa") = 150 * 1000, m_flow_nominal = 500, p_nominal = 150 * 1000)  annotation(
  ...
  Modelica.Blocks.Sources.Ramp ramp_opening_noz070(duration = 2, height = 0.1, offset = 0.9, startTime = 15) annotation(
    Placement(visible = true, transformation(origin = {100, 28}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
equation
  ...
  connect(T45.port, valveCompressible.port_a) annotation(
    Line(points = {{58, -30}, {80, -30}}, color = {0, 127, 255}));
  connect(valveCompressible.port_b, p5.port) annotation(
    Line(points = {{120, -30}, {186, -30}}, color = {0, 127, 255}));
  connect(ramp_opening_noz070.y, valveCompressible.opening) annotation(
    Line(points = {{100, 18}, {100, -14}}, color = {0, 0, 127}));
  annotation(
