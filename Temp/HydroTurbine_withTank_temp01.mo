...
model HydroTurbine_withTank_temp01
  ...
  Modelica.Fluid.Vessels.OpenTank tank(redeclare package Medium = fluid1, crossArea = Modelica.Constants.pi / 4.0 * 1.0 ^ 2, height = 10, level(fixed = false, start = 10), nPorts = 1, portsData = {Modelica.Fluid.Vessels.BaseClasses.VesselPortsData(diameter = 0.5, height = 0.0)}, use_portsData = true)  annotation(
    Placement(visible = true, transformation(origin = {-140, 0}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
equation
  connect(tank.ports[1], volumeFlowRate2.port_a) annotation(
    Line(points = {{-140, -20}, {-140, -30}, {-80, -30}}, color = {0, 127, 255}));
...
end HydroTurbine_withTank_temp01;
