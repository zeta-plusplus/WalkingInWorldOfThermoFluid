...
model FlowResistance_Chg_CauseResult_ex01
  ...
  Modelica.Fluid.Pipes.StaticPipe pipe(redeclare package Medium = fluid1, diameter = 0.05, length = 5) annotation(
    Placement(visible = true, transformation(origin = {1.77636e-15, 32}, extent = {{-20, -15}, {20, 15}}, rotation = 0)));
  Modelica.Fluid.Pipes.StaticPipe pipe1(redeclare package Medium = fluid1, diameter = 0.05, length = 5) annotation(
    Placement(visible = true, transformation(origin = {0, -66}, extent = {{-20, -15}, {20, 15}}, rotation = 0)));
equation
  ...
  connect(pipe.port_a, BC_H_1.ports[1]) annotation(
    Line(points = {{-20, 32}, {-40, 32}}, color = {0, 127, 255}));
  connect(pipe.port_b, BC_L_1.ports[1]) annotation(
    Line(points = {{20, 32}, {40, 32}}, color = {0, 127, 255}));
  connect(pipe1.port_a, BC_H_2.ports[1]) annotation(
    Line(points = {{-20, -66}, {-40, -66}}, color = {0, 127, 255}));
  connect(pipe1.port_b, BC_L_2.ports[1]) annotation(
    Line(points = {{20, -66}, {40, -66}}, color = {0, 127, 255}));
  annotation(
    experiment(StartTime = 0, StopTime = 5, Tolerance = 1e-06, Interval = 0.001),
    __OpenModelica_simulationFlags(lv = "LOG_STATS", s = "dassl"),
    Diagram(graphics = {Text(origin = {12, 70}, extent = {{-72, 6}, {72, -6}}, textString = "Cause: Upstream, downstream pressures"), Text(origin = {11, 58}, extent = {{-69, 4}, {69, -4}}, textString = "-> Result: mass flow rate"), Text(origin = {12, -26}, extent = {{-72, 6}, {72, -6}}, textString = "Cause: Upstream pressure, mass flow rate"), Text(origin = {21, -38}, extent = {{-69, 4}, {69, -4}}, textString = "-> Result: downstream pressure")}, coordinateSystem(extent = {{-100, -100}, {100, 100}})));
end FlowResistance_Chg_CauseResult_ex01;
