...
model PumpAsTurbine_ex02
  ...
  Modelica.Fluid.Sources.Boundary_pT boundary(redeclare package Medium = fluid1, T = 1000, nPorts = 1, p = 10 * 101.325 * 1000, use_p_in = true) annotation(
  ...
  Modelica.Blocks.Sources.Ramp ramp_p_in(duration = 10, height = 2 * 101.325 * 1000, offset = 10 * 101.325 * 1000, startTime = 30) annotation(
...
equation
  connect(ramp_p_in.y, boundary.p_in) annotation(
    Line(points = {{-136, 10}, {-130, 10}, {-130, -2}, {-122, -2}, {-122, -2}}, color = {0, 0, 127}));
  connect(boundary.ports[1], volumeFlowRate2.port_a) annotation(
    Line(points = {{-100, -10}, {-90, -10}}, color = {0, 127, 255}));
  connect(ramp_m_flow.y, Constraint.u_targetValue) annotation(
    Line(points = {{30, -69}, {30, -54}}, color = {0, 0, 127}));
  connect(massFlowRate1.m_flow, Constraint.u_variable) annotation(
    Line(points = {{30, -20}, {30, -20}, {30, -30}, {30, -30}}, color = {0, 0, 127}));
  connect(pressure1.port, boundary1.ports[1]) annotation(
    Line(points = {{60, -10}, {110, -10}, {110, -10}, {110, -10}}, color = {0, 127, 255}));
  ...
  annotation(
    ...
    Diagram(coordinateSystem(extent = {{-160, -120}, {140, 100}}), graphics = {Text(origin = {-2, 47}, extent = {{-18, 7}, {112, -7}}, textString = "Constrain power extracted by turbine (input)"), Text(origin = {78, -45}, extent = {{-18, 7}, {84, -9}}, textString = "downstream fluid pressure is output"), Line(origin = {65, 7}, points = {{-11, 35}, {11, -41}}, thickness = 0.5, arrow = {Arrow.None, Arrow.Open}), Line(origin = {-52.02, 31.02}, points = {{9.0238, -15.0238}, {7.0238, 12.9762}, {-8.9762, 14.9762}}, arrow = {Arrow.None, Arrow.Open}), Text(origin = {-156, 31}, extent = {{64, -1}, {112, -7}}, textString = "mechanical power")}),
