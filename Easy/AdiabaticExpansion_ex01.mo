...
model AdiabaticExpansion_ex01
  ...
  Modelica.Blocks.Sources.Constant const_p1(k = 10*100*1000) annotation(
    Placement(transformation(origin = {-18, -40}, extent = {{6, -6}, {-6, 6}})));
  ...
  Modelica.Fluid.Sensors.SpecificEntropyTwoPort specificEntropy2(redeclare package Medium = fluid1)  annotation(
    Placement(transformation(origin = {104, -54}, extent = {{-7, -7}, {7, 7}})));
  ...
  Modelica.Blocks.Interaction.Show.RealValue realValue4(significantDigits = 4) annotation(
    Placement(transformation(origin = {86, -30}, extent = {{12, -7}, {-12, 7}})));
  Modelica.Blocks.Math.Feedback dh_is annotation(
    Placement(transformation(origin = {16, 16}, extent = {{-8, 8}, {8, -8}}, rotation = -0)));
  Modelica.Blocks.Math.Product dh annotation(
    Placement(transformation(origin = {36, 12}, extent = {{-6, -6}, {6, 6}})));
  Modelica.Blocks.Sources.Constant effAdiab(k = 0.9)  annotation(
    Placement(transformation(origin = {8, -4}, extent = {{-6, -6}, {6, 6}}, rotation = -0)));
  Modelica.Blocks.Math.Feedback h2 annotation(
    Placement(transformation(origin = {48, -18}, extent = {{8, -8}, {-8, 8}}, rotation = -180)));
...
equation
  ...
  connect(const_p1.y, boundary1.p_in) annotation(
    Line(points = {{-24.6, -40}, {-36.4, -40}}, color = {0, 0, 127}));
  ...
  connect(gain.y, boundary2.m_flow_in) annotation(
    Line(points = {{-10, -66}, {-3.4, -66}, {-3.4, 76}, {42, 76}}, color = {0, 0, 127}));
  ...
  connect(boundary5.ports[1], specificEntropy2.port_a) annotation(
    Line(points = {{88, -54}, {98, -54}}, color = {0, 127, 255}));
  connect(specificEntropy2.port_b, boundary3.ports[1]) annotation(
    Line(points = {{111, -54}, {200, -54}}, color = {0, 127, 255}));
  connect(specificEntropy2.port_b, pressure1.port) annotation(
    Line(points = {{111, -54}, {170, -54}, {170, -48}}, color = {0, 127, 255}));
  ...
  connect(realValue4.numberPort, specificEntropy2.s) annotation(
    Line(points = {{100, -30}, {104, -30}, {104, -46}}, color = {0, 0, 127}));
  connect(gain.y, boundary5.m_flow_in) annotation(
    Line(points = {{-10.4, -66}, {-4.4, -66}, {-4.4, -46}, {67.6, -46}}, color = {0, 0, 127}));
  ...
  connect(specificEnthalpy.h_out, dh_is.u1) annotation(
    Line(points = {{-120, -30}, {-120, -24}, {-108, -24}, {-108, 16}, {10, 16}}, color = {0, 0, 127}));
  connect(specificEnthalpy1.h_out, dh_is.u2) annotation(
    Line(points = {{126, 34}, {126, 30}, {16, 30}, {16, 22}}, color = {0, 0, 127}));
  connect(effAdiab.y, dh.u2) annotation(
    Line(points = {{15, -4}, {20, -4}, {20, 8}, {28, 8}}, color = {0, 0, 127}));
  connect(dh_is.y, dh.u1) annotation(
    Line(points = {{24, 16}, {28, 16}}, color = {0, 0, 127}));
  connect(specificEnthalpy.h_out, h2.u1) annotation(
    Line(points = {{-120, -30}, {-120, -24}, {-108, -24}, {-108, -18}, {42, -18}}, color = {0, 0, 127}));
  connect(dh.y, h2.u2) annotation(
    Line(points = {{42, 12}, {48, 12}, {48, -12}}, color = {0, 0, 127}));
  connect(h2.y, boundary5.h_in) annotation(
    Line(points = {{56, -18}, {58, -18}, {58, -50}, {66, -50}}, color = {0, 0, 127}));
  ...
  connect(specificEntropy2.port_b, temperature2.port) annotation(
    Line(points = {{112, -54}, {124, -54}, {124, -74}}, color = {0, 127, 255}));
  connect(specificEntropy2.port_b, specificEnthalpy2.port) annotation(
    Line(points = {{112, -54}, {150, -54}, {150, -68}}, color = {0, 127, 255}));
